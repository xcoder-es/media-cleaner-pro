...
impl FileSystem for NativeFileSystem {
    ...
    async fn read_file(&self, path: &Path) -> Result<Vec<u8>, DomainError> {
        let data = tokio::fs::read(path)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("read_file {}: {}", path.display(), e)))?;
        Ok(data)
    }
    ...
    async fn write_file(&self, path: &Path, data: &[u8]) -> Result<(), DomainError> {
        if let Some(parent) = path.parent() {
            tokio::fs::create_dir_all(parent)
                .await
                .map_err(|e| DomainError::OperationFailed(format!("create_dir {}: {}", parent.display(), e)))?;
        }
        tokio::fs::write(path, data)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("write_file {}: {}", path.display(), e)))?;
        Ok(())
    }
    ...
    async fn create_dir(&self, path: &Path) -> Result<(), DomainError> {
        tokio::fs::create_dir_all(path)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("create_dir {}: {}", path.display(), e)))?;
        Ok(())
    }
    ...
    async fn delete_file(&self, path: &Path) -> Result<(), DomainError> {
        tokio::fs::remove_file(path)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("delete_file {}: {}", path.display(), e)))?;
        Ok(())
    }
    ...
    async fn copy_file(&self, src: &Path, dest: &Path) -> Result<(), DomainError> {
        ...
        tokio::fs::copy(src, dest)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("copy {} -> {}: {}", src.display(), dest.display(), e)))?;
        Ok(())
    }
    ...
    async fn move_file(&self, src: &Path, dest: &Path) -> Result<(), DomainError> {
        ...
        tokio::fs::rename(src, dest)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("move {} -> {}: {}", src.display(), dest.display(), e)))?;
        Ok(())
    }
    ...
    async fn canonicalize(&self, path: &Path) -> Result<String, DomainError> {
        let canonical = tokio::fs::canonicalize(path)
            .await
            .map_err(|e| DomainError::OperationFailed(format!("canonicalize {}: {}", path.display(), e)))?;
