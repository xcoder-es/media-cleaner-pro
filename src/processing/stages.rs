use super::{ImageMetadata, StageResult};
...
impl StageProcessor {
    ...
    pub fn icon_detection(meta: &ImageMetadata) -> StageResult {
        // Icons are typically square and small
    ...
    pub fn document_detection(meta: &ImageMetadata) -> StageResult {
        let aspect = meta.width as f32 / meta.height.max(1) as f32;
        let paper_ratios = [1.414, 1.294, 1.545, 1.0]; // A4, Letter, Legal, Square
    ...
    /// Stage 9: AI Classification (heuristic-based)
    ...
    pub fn quality_ranking(meta: &ImageMetadata) -> StageResult {
        ...
        let total = size_score + res_score + aspect_score + 15.0; // base 15
